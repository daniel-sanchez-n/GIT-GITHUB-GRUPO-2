...
typedef struct {
    char grid[MAXSIZEGRID][MAXSIZEGRIDÇ]; // La quadrícula pot ser entre 15x15 i 50x50
...
/* // Funció per inicialitzar la sopa de lletres amb dimensions donades
void initSoup(tSoup *sopa, int amplada, int alcada) {
    sopa->amplada = amplada;
    sopa->alcada = alcada;
    sopa->estat = BUIDA;  // Inicialitza la sopa com buida
    
    // Omple la sopa amb caràcters aleatoris (per exemple, lletres aleatòries)
    int i;
    for (i = 0; i < alcada; i++) {
    	int j;
        for (j = 0; j < amplada; j++) {
            sopa->grid[i][j] = 'A' + (rand() % 26);  // Lletres aleatòries entre 'A' i 'Z'
        }
    }
} */
